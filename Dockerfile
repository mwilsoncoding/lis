...
RUN apk add --no-cache libstdc++ openssl ncurses-dev musl-locales musl-locales-lang ca-certificates

# Set the locale
ENV MUSL_LOCPATH=/usr/share/i18n/locals/musl
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/lis ./
